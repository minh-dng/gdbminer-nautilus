...
- **Epsilon:** empty alternative `[]` is allowed and becomes an empty NAUTILUS
  RHS. Empty-string terminals (`[""]` or a `""` token inside an alternative)
  are **rejected** with a named `ConversionError` because the pinned JSON
  loader cannot distinguish them from epsilon.
